...
func (r *PlayerRepository) GetByID(ctx context.Context, id int64) (*Player, error) {
	...
	SELECT id, username, password_hash, level, experience, gold, bank_gold, health, max_health,
	       attack, defense, weapon_id, armor_id, potions_count, forest_fights, dragon_kills, master_fought_today, last_login_day,
...
func (r *PlayerRepository) Save(ctx context.Context, p *Player) error {
	...
	UPDATE players SET
		...
		dragon_kills = ?, master_fought_today = ?, last_login_day = ?, updated_at = CURRENT_TIMESTAMP
	WHERE id = ?
	`
	_, err := r.db.ExecContext(ctx, query,
		...
		p.DragonKills, p.MasterFoughtToday, p.LastLoginDay, p.ID,
	)
	return err
}
...
func scanPlayer(scanner rowScanner) (*Player, error) {
	...
	err := scanner.Scan(
		...
		&p.ForestFights, &p.DragonKills, &p.MasterFoughtToday, &p.LastLoginDay, &p.CreatedAt, &p.UpdatedAt,
	)
	if err != nil {
